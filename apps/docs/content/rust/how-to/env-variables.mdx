...
To configure env variables for an existing service, Rust to the service detail and choose **Environment variables** in the left menu. Click on the **Add new variable** button and set variable key and value.

Zerops creates several helper variables when a Rust service is created, e.g. `hostname`, `PATH`. Some helper variables are read-only (`hostname`), others are editable (`PATH`).
...
:::caution
You need to **restart** the runtime service after you commit the changes in env variables. The Rust process running in the container receives the list env variables only when it starts. Update of the env variables while the Rust process is running does not affect your application.
:::
...
:::caution
When you change the value of the `connectionString` variable in the service `dbtest` and commit the changes you need to **restart** the Rust service. The Rust process running in the container receives the list env variables only when it starts. Update of the env variables while the Rust process is running does not affect your application.
:::
