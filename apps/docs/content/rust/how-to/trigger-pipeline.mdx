...
The command triggers the [build pipeline](/rust/how-to/build-pipeline) defined in `zerops.yml`. `zerops.yml` must be in the working directory. The working directory is by default the current directory and can be changed using the `--workingDir` flag.
