...
- SeaweedFS mount -- `weed mount` (or `zsc shared-storage mount <hostname>`) run as a `run.startCommands` entry in zerops.yml, mounts a [SeaweedFS](/seaweedfs/overview) service's filer at a directory of your choice. There is no `mount` field in zerops.yml (a `run.mount` is silently ignored) and the import-level `mount:` of the deprecated Shared Storage is rejected. These are completely unrelated features.
