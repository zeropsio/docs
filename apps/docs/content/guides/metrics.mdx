...
Set these on your app as service env vars (GUI or `run.envVariables`). Copy the real APM server URL from the `apmserver` service's subdomain in the GUI — it's a generated subdomain (`apmserver-<subdomainHost>-<port>.<region>.zerops.app`), **not** a fixed `apmserver.zerops.app` host:
```
ELASTIC_APM_ACTIVE=true
ELASTIC_APM_SERVICE_NAME=my-app
ELASTIC_APM_SERVER_URL=https://<apmserver-subdomain-from-gui>
...
2. Set env var: `ZEROPS_PROMETHEUS_PORT=<port your /metrics endpoint listens on>` (e.g. `9090`; comma-separated for multiple ports)
