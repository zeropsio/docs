...
Readiness check gates traffic during deploy. Health check runs continuously — an unhealthy container is restarted after its configurable `failureTimeout`/`execPeriod` (set them explicitly; there is no fixed schema default), not a fixed 5-minute window.
