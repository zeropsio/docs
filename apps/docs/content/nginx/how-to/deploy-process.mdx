...
4. Starts your application using the [start command](/nginx/how-to/build-pipeline#start)
...
:::info
If your application needs to be initialized in each runtime container, add [init commands](/nginx/how-to/build-pipeline#initcommands) to <code>zerops.yml</code>.
:::

:::caution
Do not use the <code>initCommands</code> for customising your runtime environment. See [how to customise the runtime environment](/nginx/how-to/deploy-process#customise-nginx-runtime-environment).
:::
...
## Readiness checks
If your application isn't ready to handle requests right after it is started via the [start command](/nginx/how-to/build-pipeline#start), configure a [readiness check](/nginx/how-to/build-pipeline#readiness-check) in your `zerops.yml`.
...
The <a href="/nginx/how-to/build-pipeline#httpget"><code>httpGet</code></a> readiness check is successful when the URL returns HTTP status code `2xx`. The timeout is 5 seconds. When the URL returns a `3xx` HTTP status, the readiness check HTTP client will follow the redirect.

The <a href="/nginx/how-to/build-pipeline#exec"><code>exec.command</code></a> readiness check is successful when the command returns status code 0. The timeout is 5 seconds.
