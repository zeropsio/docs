...
Read [How it works](/zcp/concept/how-it-works) for the model behind what just happened: live state, runtime fit, app wiring, deploy evidence, behavior proof, and blockers.

Read [Remote or local setup](/zcp/setup/choose-workspace) when you are deciding where the agent workspace should live. Remote setup keeps the agent inside Zerops; local setup keeps it next to your files, editor, data, and tools.

Read [Build and ship](/zcp/workflows/build-with-zcp) before giving the agent larger tasks. It explains the practical choices you control during normal work: runtime layout, app request, delivery, packaging, and production promotion.

Before promoting anything to production, read [Promote to production](/zcp/workflows/promote-to-production). ZCP belongs in development or staging; production should receive verified work through your release process.
