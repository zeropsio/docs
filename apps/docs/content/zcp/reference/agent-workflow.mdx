...
| `zerops_knowledge` | Surface MCP-side guidance specific to current project state and step |

### Destructive — mutate the project or its services

Host agent should require user confirmation. Two operations carry an explicit MCP-side gate (see Confirmation gates).
...
Two operations carry an explicit MCP-side confirmation gate because the loss isn't reversible from inside the conversation:

- **Service deletion** — requires explicit user approval in the same conversation, by service name. The hosted MCP additionally hard-blocks deleting the `zcp` service it's running on (`SELF_SERVICE_BLOCKED`).
