...
**ZCP MCP tools** - the bounded Zerops operations exposed to an agent or MCP-capable client. In MCP clients, this usually appears as the `zerops` server.
...
**Production promotion** - handoff from verified dev or stage work into a separate production Zerops project through production infrastructure setup, production deploy trigger setup, and repeatable releases.
