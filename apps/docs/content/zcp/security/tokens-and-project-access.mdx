...
| `ZEROPS_TOKEN`               | `zcli` in GitHub Actions or external CI      | Separate project-scoped Zerops token stored in the CI/release secret store. |
...
For ZCP setup, provide `ZCP_API_KEY`. `zcli` login is a diagnostic fallback, not the normal agent setup path.
...
`ZEROPS_TOKEN` is a Zerops API token used by GitHub Actions or another CI system when that system runs `zcli` against Zerops. It is not a GitHub token. Use a separate project-scoped delivery token so ZCP sessions and CI/release workflows can be named, rotated, and audited independently.

For production delivery, `ZEROPS_TOKEN` should be scoped to the production project and stored only in the production CI or release secret store.
...
| -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| Remote `ZCP_API_KEY` | Reconfigure or redeploy the remote workspace through the dashboard-managed path, then restart the `zcp` service so the process gets the new value. |
...
- [Use remote workspace](/zcp/setup/hosted-workspace) - automatic token injection.
