...
For ZCP setup, provide `ZCP_API_KEY`. `zcli` login is a diagnostic fallback, not the normal agent setup.
...
- [How to run ZCP on your machine](/zcp/setup/local-agent-bridge) - local `.mcp.json` token setup.
