---
title: Stop and start PostreSQL service
---

Zerops allows you to stop any service. Stopped services only consume disk.

## Stop and start PostreSQL service in Zerops GUI
To stop the PostreSQL service in Zerops GUI go to the project dashboard and select the stop menu item in the top right corner.

**// TODO screenshot (show menu with the start and stop items on a service)**

To start the stopped PostreSQL service choose the **start** item from the same menu.

## Stop and start PostreSQL using zCLI
zCLI is the Zerops command-line tool. To stop and start the PostreSQL service via the command-line, follow these steps:
