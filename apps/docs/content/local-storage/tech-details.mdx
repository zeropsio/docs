...
:::note
Even though locking is fully correct, a single-writer setup remains the most robust pattern for SQLite and similar databases. Use the [`readOnly` mount option](/local-storage/how-to/connect#the-volume-field) to enforce it.
:::
...
Local Storage is **not highly available** — the volume exists once, on one machine, with no replication.
