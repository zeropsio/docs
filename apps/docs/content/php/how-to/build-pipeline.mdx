...
// FIXME co je required a co optional
```yml
zerops:
...
Each service configuration contains two sections: **build** and **run**. Both sections are required to build and deploy your PHP application in Zerops.
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `buildCommands` are finished, the application build is completed and ready for the [deploy](/php/how-to/build-pipeline#deploy) phase.
...
If you need to upgrade the major PHP version, create a new service with the required version and set a new [hostname](/php/how-to/build-pipeline#hostname) in your `zerops.yml`.
