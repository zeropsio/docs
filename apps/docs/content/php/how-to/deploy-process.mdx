...
If you triggered the deploy pipeline [manually](/php/how-to/trigger-pipeline#manual-builds-and-deploys-using-zerops-cli) using Zerops CLI, the application artefact is also uploaded to the internal Zerops storage.
...
3. Optionally runs the [init commands](/php/how-to/build-pipeline#init)
4. Starts your application using the [start command](/php/how-to/build-pipeline#start)
...
:::info
If your application needs to be initialized in each runtime container, add [init commands](/php/how-to/build-pipeline#init) to <code>zerops.yml</code>.
:::

:::caution
Do not use the <code>initCommands</code> for customising your runtime environment. See [how to customise the runtime environment](/php/how-to/deploy-process#customise-php-runtime-environment).
:::
...
## Readiness checks
If your application isn't ready to handle requests right after it is started via the [start command](/php/how-to/build-pipeline#start), configure a [readiness check](/php/how-to/build-pipeline#readiness-check) in your `zerops.yml`.
...
The <a href="/php/how-to/build-pipeline#httpget"><code>httpGet</code></a> readiness check is successful when the URL returns HTTP status code `2xx`. The timeout is 5 seconds. When the URL returns a `3xx` HTTP status, the readiness check HTTP client will follow the redirect.

The <a href="/php/how-to/build-pipeline#exec"><code>exec.command</code></a> readiness check is successful when the command returns status code 0. The timeout is 5 seconds.
