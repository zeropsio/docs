...
If you prefer the Ubuntu OS instead of Alpine, set the `<a href="/php/how-to/build-pipeline#os">build.os</a>` attribute to `ubuntu`. To install additional packages or tools add one or more [build.prepareCommands](/php/how-to/build-pipeline#preparecommands) commands to your `zerops.yml`.
