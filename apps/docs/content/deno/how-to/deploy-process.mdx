...
:::info
If your application needs to be initialized in each runtime container, add [init commands](/deno/how-to/build-pipeline#initcommands) to `zerops.yml`.
:::
...
If your application isn't ready to handle requests right after it is started via the [start command](/deno/how-to/build-pipeline#start), configure a [readiness check](/deno/how-to/build-pipeline#readiness-check) in your `zerops.yml`.
