...
<UnorderedCodeList data={data.denoVersions} />
...
See the full list of supported [build base environments](/references/zeropsyml/base-list#deno).
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `buildCommands` are finished, the application build is completed and ready for the [deploy](/deno/how-to/build-pipeline#deploy) phase.
...
<p>
  ...
  selected major version of Deno, Zerops command line tool, `npm`,{' '}
  `yarn`, `git` and `npx` tools.
</p>
...
<p>
  The base Deno environment contains {data.alpineCurrentVersion} the selected
  major version of Deno,{' '}
...
<p>
  ...
  selected major version of Deno,{' '}
  <a href="/references/cli">Zerops command line tool</a> and `npm`,{' '}
  `yarn`, `git` and `npx` tools.
</p>
...
:::caution
The init commands will delay the start of your application each time a new runtime container is started (including the [horizontal scaling](/deno/how-to/scaling#horizontal-scaling) or when a runtime container is restarted.
...
| Parameter   | Description                                                                                                                                                                                                                                                                                                                     |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **command** | Defines a local command to be run.<br/>The command has access to the same [environment variables](/deno/how-to/create#set-environment-variables) as your Deno application.<br/>A single string is required. If you need to run multiple commands create a shell script or, use a multiline format as in the example below. |
