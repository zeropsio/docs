...
3. Optionally [customises the build environment](/deno/how-to/build-process#customise-deno-build-environment)
4. Runs the build commands
5. Uploads the application artefact to the internal Zerops storage
6. Optionally [customizes the runtime environment](/deno/how-to/customize-runtime)
7. [Deploys your application](/deno/how-to/deploy-process)
...
<p align="center">
  <img
    src="/img/screenshots/deno_cancel_running_build.png"
    alt="image"
    width="100%"
    height="auto"
  />
...
## Customise Deno build environment

The default Deno build environment contains:

- <span>{data.alpine.default}</span>
- selected version of Deno defined in `zerops.yml` [build.base](/deno/how-to/build-pipeline#base) parameter
- [zCLI](/references/cli), Zerops command line tool
- `npm`, `yarn`, `git` and `npx` tools

If you prefer the Ubuntu OS instead of Alpine, set the `<a href="/deno/how-to/build-pipeline#os">build.os</a>` attribute to `ubuntu`. To install additional packages or tools add one or more [build.prepareCommands](/deno/how-to/build-pipeline#preparecommands) commands to your `zerops.yml`.
...
1. Content of the [build.base](/deno/how-to/build-pipeline#base) and [build.prepareCommands](/deno/how-to/build-pipeline#preparecommands) attributes didn't change from the previous build
...
If any [build command](/deno/how-to/build-pipeline#build) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/deno/how-to/logs#build-log) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the `--verbose` option.
