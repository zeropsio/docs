---
title: Configure Your Elixir build & deploy pipeline
...
{/* <UnorderedCodeList data={data.elixirVersions} /> */}
...
See the full list of supported [build base environments](/references/zeropsyml/base-list#elixir).
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `buildCommands` are finished, the application build is completed and ready for the [deploy](/elixir/how-to/build-pipeline#deploy) phase.
...
<p>
  ...
  selected major version of Elixir, Zerops command line tool, `npm`,{' '}
  `yarn`, `git` and `npx` tools.
</p>
...
<p>
  The base Elixir environment contains {data.alpineCurrentVersion} the selected
  major version of Elixir,{' '}
...
<p>
  ...
  selected major version of Elixir,{' '}
  <a href="/references/cli">Zerops command line tool</a> and `npm`,{' '}
  `yarn`, `git` and `npx` tools.
</p>
...
:::caution
The init commands will delay the start of your application each time a new runtime container is started (including the [horizontal scaling](/elixir/how-to/scaling#horizontal-scaling) or when a runtime container is restarted.
...
| Parameter   | Description                                                                                                                                                                                                                                                                                                                     |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **command** | Defines a local command to be run.<br/>The command has access to the same [environment variables](/elixir/how-to/create#set-environment-variables) as your Elixir application.<br/>A single string is required. If you need to run multiple commands create a shell script or, use a multiline format as in the example below. |
