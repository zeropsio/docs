...
zerops:
  ...
  - setup: app
    ...
    run:
      ...
      ports:
        - port: 3000
          httpSupport: true   # default: false — enable for HTTP/S traffic
...
<table className="w-full my-1.5">
  ...
  <tbody>
    <tr>
      ...
      <td className="w-fit">Defines the port number. You can set any port number between _10_ and _65435_. Ports outside this interval are reserved for internal Zerops systems.</td>
    </tr>
    <tr>
      <td className="w-fit">protocol</td>
      <td className="w-fit">**Optional.** Defines the protocol. Allowed values are `tcp` or `udp`. Default value is `tcp`.</td>
    </tr>
    <tr>
      <td className="w-fit">httpSupport</td>
      <td className="w-fit">**Optional.** `httpSupport` defaults to `false`. Set to `true` if a web server is running on the port — required for HTTP/S public access via subdomains or custom domains. Available only with the `tcp` protocol.</td>
    </tr>
  </tbody>
</table>
...
<table className="w-full my-1.5">
  ...
  <tbody>
    <tr>
      <td className="w-fit">**command**</td>
      <td className="w-fit">Defines a local command to be run.<br/>The command has access to the same [environment variables](/elixir/how-to/create#set-secret-environment-variables) as your Elixir application.<br/>A single string is required. If you need to run multiple commands create a shell script or, use a multiline format as in the example below.</td>
    </tr>
  </tbody>
</table>
