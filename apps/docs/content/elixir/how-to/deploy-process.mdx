...
:::info
If your application needs to be initialized in each runtime container, add [init commands](/elixir/how-to/build-pipeline#initcommands) to `zerops.yml`.
:::

:::caution
Do not use the `initCommands` for customising your runtime environment. See [how to customise the runtime environment](/elixir/how-to/build-pipeline#preparecommands-1).
:::
...
If your application isn't ready to handle requests right after it is started via the [start command](/elixir/how-to/build-pipeline#start), configure a [readiness check](/elixir/how-to/build-pipeline#readiness-check) in your `zerops.yml`.
