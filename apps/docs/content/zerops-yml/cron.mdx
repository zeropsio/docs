...
### command <Badge type="custom" text="string" /> <Badge type="required" />

The shell command to execute at the scheduled time. This can be any valid command.

### timing <Badge type="custom" text="string" /> <Badge type="required" />
...
### allContainers <Badge type="custom" text="boolean" /> <Badge type="required" /> 

**Options:**
- `true` – Command runs on all containers.
- `false` – Command runs on only one container.

### workingDir <Badge type="custom" text="string" /> <Badge type="optional" />
