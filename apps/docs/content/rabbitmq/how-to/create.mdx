...
| ---------------------- | ----------------------------------------------------------------------------- | ----------- |
| **hostname**           | A unique service identifier like `rabbitmq`,`sql`, `db` etc.                   | - duplicate services with the same name in the same project are forbidden<br/>- maximum 25 characters<br/>- must contain only lowercase ASCII letters (a-z) or numbers (0-9) 
...
Creates a RabbitMQ cluster with **3 database containers** and **2 free database proxies**. 
...
[Learn more](/rabbitmq/tech-details/cluster) about specific behaviour and technical limitations of the RabbitMQ cluster.

#### Single container

A RabbitMQ database installed in a single container is created. Useful for non-essential data or dev environments.
...
Zerops scales the RabbitMQ services automatically by raising or lowering the hardware resources of each database container.
...
RabbitMQ needs at least 10 vCPUs to start. Zerops automatically starts RabbitMQ service with at least 10 vCPUs. If the database is idle after the start, Zerops will scale the RabbitMQ down to selected minimum vCPU settings. If you set the minimum vCPU settings higher than 10 vCPUs, Zerops will start with the required minimum vCPU settings. 
...
project:
  ...
  description: A project with a RabbitMQ database
...
<tbody>
	...
	<tr>
  <td><b>type</b></td>
		<td>Specifies the service type <code>rabbitmq</code> and version.<br/>Set type: <code>rabbitmq<span>@</span>XXX</code></td>
		<td>Currently only RabbitMQ <i>XXX</i> is available.</td>
	</tr>
  	<tr>
  <td><b>mode</b></td>
		<td>Defines the operation mode of RabbitMQ service.<br/>
    <br/><b>HA</b><br/>Zerops will create a RabbitMQ cluster with 3 database containers and 2 free database proxies. This mode is suited for production.<br/><br/>
    ...
    <b>NON_HA</b><br/>
    Zerops will create a RabbitMQ database installed in a single container. Useful for non-essential data or dev environments.<br/><br/>
    Your data is stored only in a single container. If the container or the underlying physical machine fails, your data since the last backup are lost. Zerops doesn't provide any automatic repairs of single node RabbitMQ services.
    </td>
		<td><a href="/rabbitmq/tech-details/limitations">Learn more</a> about specific behaviour and technical limitations of the RabbitMQ cluster.</td>
	</tr>
  	<tr>
  <td><b>verticalAutoscaling</b></td>
		<td><b>Optional.</b> Defines <a href="/rabbitmq/how-to/scale#vertical-scaling">custom auto scaling parameters</a>.</td>
		<td></td>
	</tr>
</tbody>
</table>
