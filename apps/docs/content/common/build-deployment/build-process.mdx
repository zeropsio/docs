---
title: Build process
description: Learn more about build process.
---
...
3. Optionally [customizes the build environment](/common/build-deployment/build-process#customize-build-environment)
4. Runs the build commands
5. Uploads the application artefact to the internal Zerops storage
6. Optionally [customizes the runtime environment](/common/build-deployment/customize-runtime)
7. [Deploys your application](/common/build-deployment/deploy-process)
...
- OS - either defined in `zerops.yml` [build.os](/common/build-deployment/build-pipeline#base) parameter or technology-specific version of Alpine or Ubuntu
- selected version of runtime technology defined in `zerops.yml` [build.base](/common/build-deployment/build-pipeline#base) parameter
- [zCLI](/references/cli), Zerops command line tool
- `git` and other technology-specific tools

To install additional packages or tools add one or more [build.prepareCommands](/common/build-deployment/build-pipeline#preparecommands) commands to your `zerops.yml`.
...
Build of your application is run in a separate build container with following resource configuration:
...
1. Content of the [build.base](/common/build-deployment/build-pipeline#base) and [build.prepareCommands](/common/build-deployment/build-pipeline#preparecommands) attributes didn't change from the previous build
...
If any [build command](/common/build-deployment/build-pipeline#build) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/common/maintenance/logs#build-log) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the verbose option, e.g. for Node.js:
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `buildCommands` are finished, the application build is completed and ready for the [deploy](/common/build-deployment/deploy-process) phase.
