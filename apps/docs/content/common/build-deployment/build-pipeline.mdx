---
title: Configure Your build & deploy pipeline
description: Learn more about how you can configure your service for build & deploy pipeline.
---
...
:::tip
Refer to technology-specific pages to get customized `zerops.yml` examples.

See full `zerops.yml` [specification](/zerops-yml/specification).
:::

The top-level element in `zerops.yml` is always `zerops`.
...
See the [full list](/zerops-yml/base-list#runtime-services) of supported base technologies.
...
zerops:
  ...
  - setup: app
    ...
    build:
      # REQUIRED. Sets the base technology for the build environment (Node.js example)
      base: nodejs@latest
      ...
```

<p>
  The base build environment contains a chosen OS (defined in <a href="#os"><code>build.os</code></a>), the selected
  major version of your technology, <a href="/references/cli">Zerops command line tool</a> and other technology-specific tools.
</p>
...
To customize your build environment use the [prepareCommands](/common/build-deployment/build-pipeline#preparecommands) attribute.
...
zerops:
  ...
  - setup: app
    # ==== how to build your application ====
    build:
      # REQUIRED. Set the base technology for the build environment (Node.js example)
      base: nodejs@latest
...
If any command fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/common/maintenance/logs#build-log) to troubleshoot the error. If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all prepare commands are finished, your custom build environment is ready for the build phase.
...
If any command fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/common/maintenance/logs#build-log) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the `--verbose` option.
...
To speed the second and following build up, cache the files that can be used in the next build. We recommend caching the `node_modules` directory.
...
Read more about [environment variables](/common/configuration/env-variables) in Zerops.

## Runtime configuration

### base

_OPTIONAL._ Sets the base technology for the runtime environment. See all [supported base technologies](/zerops-yml/base-list#runtime-services).
If you don't specify the `run.base` attribute, Zerops keeps the current version for your runtime.
...
zerops:
  ...
  - setup: app
    ...
    run:
      # OPTIONAL. Sets the base technology for the runtime environment (Node.js example)
        base: nodejs@latest
      ...
```

<p>
  The base runtime environment contains {data.alpine.default}, the selected version of your technology, Zerops command line tool, `git` and other technology-specific tools.
</p>
...
For example, to connect to a Node.js service with hostname = "app" and port = 3000 from another service of the same project, simply use `app:3000`. Read more about [how to access a service](/common/maintenance/access).
...
_OPTIONAL._ Customizes the runtime environment by installing additional dependencies or tools to the runtime base environment.

<p>
  The base environment contains {data.alpine.current}, the selected
  major version of your technology, <a href="/references/cli">Zerops command line tool</a> and `git` and other technology-specific tools. To install
...
zerops:
  ...
  - setup: app
    ...
    # ==== how to run your application ====
    run:
      # OPTIONAL. Customize the runtime environment by installing additional packages
      # or tools to the base runtime environment.
      prepareCommands:
        - apt-get something
        - curl something else
      ...
...
2. optionally: [copy selected folders or files from your build container](/common/build-deployment/build-pipeline#copy-folders-or-files-from-your-build-container)
3. run the `prepareCommands` commands in the defined order

#### Command exit code

If any command fails, it returns an exit code other than 0 and the deploy is canceled. Read the [prepare runtime log](/common/build-deployment/logs#prepare-runtime-log) to troubleshoot the error. If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `prepareCommands` commands are finished, your custom runtime environment is ready for the deploy phase.
...
<p>
  ...
  selected major version of your technology, <a href="/references/cli">Zerops command line tool</a> and `git` and other technology-specific tools.
</p>
...
These commands are triggered in the runtime container before your application is started via the [start command](/common/build-deployment/build-pipeline#start) ().

Use init commands to clean or initialise your application cache or similar operations.

:::caution
The init commands will delay the start of your application each time a new runtime container is started (including the [horizontal scaling](/common/maintenance/scaling#horizontal-auto-scaling) or when a runtime container is restarted).

Do not use the init commands for customising your runtime environment. Use the [run:prepareCommands](/common/build-deployment/build-pipeline#preparecommands-1) attribute instead.
:::

#### Command exit code

If any of the `initCommands` fails, it returns an exit code other than 0, but deploy is **not** canceled. After all init commands are finished, regardless of the status code, the application is started. Read the [runtime log](/common/maintenance/logs#runtime-log) to troubleshoot the error.
...
_REQUIRED._ Defines the start command for your application.
...
zerops:
  ...
  - setup: app
    ...
    # ==== how to run your application ====
    run:
      # REQUIRED. Your application start command (Node.js example)
      start: npm start
```
  ...
  <tbody>
    <tr>
      ...
      <td className="w-fit">
        ...
        The command has access to the same <a href="/common/management/create#set-secret-environment-variables">environment variables</a> as your application.<br/>
...
_OPTIONAL._ Defines a readiness check. Read more about how the [readiness check works](/common/build-deployment/deploy-process#readiness-checks) in Zerops.
...
Read more about how the [readiness check works](/common/build-deployment/deploy-process#readiness-checks) in Zerops.
