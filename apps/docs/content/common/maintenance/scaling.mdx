---
title: How Zerops scales services
description: Get to know how Zerops scales your service.
---
...
To change the auto scaling settings go to the service detail and choose **Automatic scaling configuration** in the left menu.
...
All services always starts with the minimal resources.

For most cases, the default parameters will work without issues. If you need to limit the cost of the service, lower the maximal resources. Zerops will never scale above the selected maximums.

When you are experiencing problems with insufficient performance or capacity, increase the minimal resources. Zerops will never scale below the selected minimums.
...
All services always starts with the minimum number of containers.
...
Zerops prefers vertical scaling over horizontal scaling because vertical scaling is faster and allows finer adjustment to the required performance. Horizontal scaling can be disabled by setting the same number for the minimum and maximum container count. Zerops will then scale the service only vertically.
...
Zerops provides information about how much hardware resources the service is currently using. Go to the service detail in Zerops GUI and select **Service dashboard & runtime containers** in the left menu. Zerops also provides the history of resource usage.
