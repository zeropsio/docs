---
title: Stop and start MongoDB service
---

Zerops allows you to stop any service. Stopped services only consume disk.

## Stop, start and restart MongoDB service in Zerops GUI
To stop the MongoDB service in Zerops GUI go to the project dashboard and select the **Stop** menu item in the top right corner.

**// TODO screenshot (show menu with the start and stop items on a service)**

To start the stopped MongoDB service choose the **Start** item from the same menu.

To restart the MongoDB service choose the **Restart** item from the same menu.

## Stop and start MongoDB using zCLI
zCLI is the Zerops command-line tool. To stop and start the MongoDB service via the command-line, follow these steps:
