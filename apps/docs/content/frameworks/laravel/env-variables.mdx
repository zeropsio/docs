...
You can just add the `redis` environment variables which you can get from the `valkey` service and later you can add it in `zerops.yml`.
...
:::caution
You can only generate environment variables for your zerops service using the `#yamlPreprocessor=on` with `<@generateRandomString(<32>)>`.
:::
