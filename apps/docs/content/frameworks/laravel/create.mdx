...
project:
  ...
  description: A project with a Nginx service
  # optional: production project core package
  corePackage: SERIOUS
...
The yaml file describes your future project infrastructure. The project will contain an Nginx service with `app` hostname. The internal port(s) the service listens on will be defined later in the [zerops.yml](/nginx/how-to/build-pipeline#ports). Nginx service will run on version 1.22 with a custom vertical and horizontal scaling. Following secret env variables will be configured:
