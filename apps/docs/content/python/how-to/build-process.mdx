...
3. Optionally [customises the build environment](/python/how-to/build-process#customise-python-build-environment)
4. Uploads the application artefact to the internal Zerops storage
5. Optionally [customizes the runtime environment](/python/how-to/customize-runtime)
6. [Deploys your application](/python/how-to/deploy-process)
...
## Customise Python build environment
...
To install additional packages or tools add one or more [build.prepareCommands](/python/how-to/build-pipeline#preparecommands) commands to your `zerops.yml`.
...
1. Content of the [build.base](/python/how-to/build-pipeline#base) and [build.prepareCommands](/python/how-to/build-pipeline#preparecommands) attributes didn't change from the previous build
