...
Each service configuration contains two sections: **build** and **run**. Both sections are required to build and deploy your Python application in Zerops.
...
The base build environment contains Ubuntu 22, the selected major version of Python, [Zerops command line tool](/references/cli), `pip` and `git`.
...
If you need to upgrade the major Python version, create a new service with the required version and set a new [hostname](/python/how-to/build-pipeline#hostname) in your `zerops.yml`.
...
The base Python environment contains Ubuntu 22, the selected major version of Python, [Zerops command line tool](/references/cli), `pip` and `git`. To install additional packages or tools add one or more prepare commands:
...
The prepare runtime container contains Ubuntu 22, the selected major version of Python, [Zerops command line tool](/references/cli), `pip` and `git`.
