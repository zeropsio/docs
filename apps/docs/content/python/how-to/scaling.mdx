...
The scale up of RAM or disk is immediate. If Zerops detects that the container has less than 0.125 GB RAM free or less than 3 GB free disk space, Zerops will assign more RAM or disk to the container immediately.
...
Maximal resources are defined for each Python service. Zerops will never scale above the entered values. If your application is in [highly available](/python/how-to/scaling#horizontal-scaling) mode, maximal resources are identical for all containers of the Python service.
...
If you need to limit the cost of the Python service, lower the maximal resources. You can set your own maximal resources when creating a Python service or you can change it later in Zerops GUI. Go to the Python service detail and choose **Automatic scaling configuration** in the left menu.

**// TODO screenshot (Python - Automatic scaling configuration form)**
...
Minimal resources are identical for all containers of the Python service.

:::tip
When you are experiencing problems with insufficient Python performance or capacity, increase the minimal resources. You can set your own minimal resources when creating a Python service or you can change it later in Zerops GUI. Go to the Python service detail and choose **Automatic scaling configuration** in the left menu.
:::
...
To disable the vertical auto scaling of vCPU, RAM or disk, set the minimal and maximal resources to the same value. Zerops will set the required amount of resources to all Python containers and the auto scaling of the selected resource will be disabled. You can disable the auto scaling of a resource (e.g. RAM) and keep auto scaling of other resources enabled.

## Horizontal scaling

Horizontal scaling means adding or removing whole containers. When creating a new Python service, you can choose the minimum and maximum number of containers. You can change the settings anytime in Zerops GUI. Go to the Python service detail and choose **Automatic scaling configuration** in the left menu.
...
Zerops prefers [vertical scaling](/python/how-to/scaling#vertical-scaling) over horizontal scaling because vertical scaling is faster and allows finer adjustment to the required performance. Horizontal scaling can be disabled by setting the same number for the minimum and maximum container count. Zerops will then scale the Python service only vertically.
