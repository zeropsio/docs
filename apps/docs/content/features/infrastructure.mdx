...
description: Explore Project & Services Structure in Zerops.
---
...
When creating a project, you can choose the region where it will be hosted:

<table className="w-full my-1.5">
  <thead>
    <tr>
      <th className="w-fit">Value</th>
      <th className="w-fit">Region</th>
      <th className="w-fit">Details</th>
    </tr>
  </thead>
  <tbody>
    <tr>
      <td className="w-fit whitespace-nowrap">`eu-central`</td>
      <td className="w-fit">EU Central (default)</td>
      <td className="w-fit">Prague, Czech Republic — Zerops' own bare-metal data center operated by [vshosting.eu](https://vshosting.eu)</td>
    </tr>
    <tr>
      <td className="w-fit whitespace-nowrap">`us-east-1`</td>
      <td className="w-fit">US East</td>
      <td className="w-fit">New York — US East Coast infrastructure</td>
    </tr>
  </tbody>
</table>

The location defaults to **`eu-central`** and cannot be changed after project creation. Choose based on where the majority of your users are located to minimize latency.

To set the location when importing a project via YAML:

```yaml
project:
  name: my-project
  location: eu-central   # eu-central or us-east-1
```
