...
- Connect your laptop to a Zerops project → [VPN reference](/references/networking/vpn)
- Use a native editor over SSH → [SSH reference](/references/networking/ssh)
- Build and deploy from the same project model → [Build & deploy pipeline](/features/pipeline)
- Put a coding agent on top of this infrastructure → [Infrastructure for Coding Agents](/features/coding-agents)
