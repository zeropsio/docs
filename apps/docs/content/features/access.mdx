...
Once enabled, Zerops assigns a unique subdomain for your application. If you've defined multiple [internal ports](/zerops-yml/specification#ports-) with HTTP support in your `zerops.yml`, each port receives its own unique `.zerops.app` subdomain.
