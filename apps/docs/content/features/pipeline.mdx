...
To install additional packages or tools add one or more [build.prepareCommands](/zerops-yml/specification#preparecommands-optional) commands to your `zerops.yml`.
...
Each runtime service in Zerops has a default runtime environment with a major version based on the [run.base](/zerops-yml/specification#base-optional) attribute in `zerops.yml`.
To install additional packages or tools add one or more [run.prepareCommands](/zerops-yml/specification#preparecommands-optional-1) commands to your `zerops.yml`.
...
When the first deploy with a defined [prepareCommands](/zerops-yml/specification#preparecommands-optional-1) attribute is triggered, Zerops will

1. create a prepare runtime container
2. optionally: [copy selected folders or files from your build container]
3. run the [run.prepareCommands](/zerops-yml/specification#preparecommands-optional-1) commands in the defined order
...
1. Content of the [build.addToRunPrepare](/zerops-yml/specification#addtorunprepare-optional) and [run.prepareCommands](/zerops-yml/specification#preparecommands-optional-1) attributes didn't change from the previous deploy
...
<p align="center">
  <Image
    lightImage="/img/screenshots/cancel_running_build.png"
    darkImage="/img/screenshots/cancel_running_build.png"
    alt="Running build"
    style={{ width: '95%', height: 'auto' }}
  />
</p>
