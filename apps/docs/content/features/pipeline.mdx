...
Each runtime service in Zerops has a default build environment with a major version based on the [build.base](/references/zeropsyml#base-required) attribute in `zerops.yml`.
To install additional packages or tools add one or more [build.prepareCommands](/references/zeropsyml#preparecommands-optional) commands to your `zerops.yml`.
...
Each runtime service in Zerops has a default runtime environment with a major version based on the [run.base](/references/zeropsyml#base-optional) attribute in `zerops.yml`.
To install additional packages or tools add one or more [run.prepareCommands](/references/zeropsyml#preparecommands-optional-1) commands to your `zerops.yml`.
...
When the first deploy with a defined [prepareCommands](/references/zeropsyml#preparecommands-optional-1) attribute is triggered, Zerops will
...
1. Content of the [build.addToRunPrepare](/references/zeropsyml#addtorunprepare-optional) and [run.prepareCommands](/references/zeropsyml#preparecommands-optional-1) attributes didn't change from the previous deploy
