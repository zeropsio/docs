...
A **Zerops project** provides a private network enabling seamless internal communication and shared access to environment variables among services, You've the flexibility to:
...
The total deployment cost includes the **core package cost** plus the **resource costs** of services within the project, along with any additional charges **(e.g. Unique IPv4, Extra Egress, Object Storage, Backup Space, Build Time, etc)**
 
:::info Costs
Core Package Cost + Resource Costs + Additional Charges
:::
...
Resources are allocated per service and billed by the minute, with hourly credit deductions based on actual usage.
...
Projects start with a shared IPv4 address. A unique IPv4 address is required for Cloudflare's proxy features.
:::

:::info Object Storage
Ideal for scalable file, backup, and static asset storage outside main storage.
:::
