...
:::note
[Local Storage](/local-storage/overview) is available in single container mode only — its volume lives on a single physical machine by design. See its [technical details](/local-storage/tech-details) for how availability and scaling work there.
:::
