...
description: Learn how you can setup and use environment variables in a gleam service on Zerops.
---
...
| ----------- | ------ | --------------------------------------------------------------------------------- |
| build       | basic  | [zerops.yml](/gleam/how-to/build-pipeline#envvariables)                          |
| runtime     | basic  | [zerops.yml](/gleam/how-to/build-pipeline#envvariables-1)                        |
| runtime     | secret | [Zerops GUI](/gleam/how-to/env-variables#set-secret-env-variables-in-zerops-gui) |
...
:::caution
You need to **restart** the runtime service after you commit the changes in env variables. The Gleam process running in the container receives the list env variables only when it starts. Update of the env variables while the Gleam process is running does not affect your application.
:::
...
:::caution
When you change the value of the `connectionString` variable in the service `dbtest` and commit the changes you need to **restart** the Gleam service. The Gleam process running in the container receives the list env variables only when it starts. Update of the env variables while the Gleam process is running does not affect your application.
:::
