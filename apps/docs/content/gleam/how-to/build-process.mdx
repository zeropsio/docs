...
3. Optionally [customises the build environment](/gleam/how-to/build-process#customise-gleam-build-environment)
4. Runs the build commands
5. Uploads the application artefact to the internal Zerops storage
6. Optionally [customizes the runtime environment](/gleam/how-to/customize-runtime)
7. [Deploys your application](/gleam/how-to/deploy-process)
...
<p align="center">
  <img
    src="/img/screenshots/gleam_cancel_running_build.png"
    alt="image"
    width="100%"
    height="auto"
  />
...
## Customise Gleam build environment

The default Gleam build environment contains:

- <span>{data.alpine.default}</span>
- selected version of Gleam defined in `zerops.yml` [build.base](/gleam/how-to/build-pipeline#base) parameter
- [zCLI](/references/cli), Zerops command line tool
- `npm`, `yarn`, `git` and `npx` tools

If you prefer the Ubuntu OS instead of Alpine, set the `<a href="/gleam/how-to/build-pipeline#os">build.os</a>` attribute to `ubuntu`. To install additional packages or tools add one or more [build.prepareCommands](/gleam/how-to/build-pipeline#preparecommands) commands to your `zerops.yml`.
...
1. Content of the [build.base](/gleam/how-to/build-pipeline#base) and [build.prepareCommands](/gleam/how-to/build-pipeline#preparecommands) attributes didn't change from the previous build
...
If any [build command](/gleam/how-to/build-pipeline#build) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/gleam/how-to/logs#build-log) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the `--verbose` option.
