...
### Choose .NET version
Following .NET major versions are currently supported:
* 6

:::caution
The .NET major version is fixed after the service is created. It can't be changed later.
:::
...
Specify one or more internal ports on which your application will listen. The TCP port **5000** is predefined. You can set your own port number, configure multiple ports or create a service without an internal port.
...
:::info
Internal .NET service ports can be changed later.
:::
...
#### How to read env variables from .NET

Zerops passes all environment variables from all project services when your .NET is deployed and started.
...
Read more about how to create and use [environment variables](/dotnet/how-to/env-variables).
...
:::caution
Check if your application is ready for [highly available mode](/dotnet/how-to/scaling#horizontal-scaling). If not, set the maximum containers to **1**. Zerops will never add more containers to your .NET service.
:::
...
The yaml file describes your future project infrastructure. The project will contain one .NET version 6 service with default [auto scaling](/dotnet/how-to/scaling) configuration. Hostname will be set to "app" and the application will listen on the TCP port 5000 that supports the HTTP communication. Following env variables will be configured:
...
.NET service with "app" hostname is configured to listen on tcp ports 5000, 5001 and on udp port 6000. .NET service will run on version 6 with a custom vertical and horizontal scaling. Following env variables will be configured:
...
At least one service in `services:` section is required. You can create a project with multiple services. The example above contains .NET and PostgreSQL services but you can create a `description.yml` with your own combination of [services].
...
<tbody>
	...
	<tr>
  <td><b>type</b></td>
		<td>Specifies the service type <code>dotnet</code> and version.<br/>Set type: <code>dotnet<span>@</span>&lt;version&gt;</code><br/><br/><b>Limitations:</b><br/>Following .NET major versions are currently supported: <i>6</i></td>
	</tr>
    <tr>
  <td><b>ports</b></td>
...
The <b>port</b> attribute defines the port number. At least one internal port is required for .NET service. You can set any port number between <code>10</code> and <code>65435</code>. Ports outside this interval are reserved for internal Zerops systems.
<br/><br/>
The <b>protocol</b> attribute is optional. Default value is <code>"TCP"</code>.<br/><br/>
httpSupport = true is the default setting for TCP protocol. Set <b>httpSupport = false</b> if a web server doesn't runs on the port. Zerops uses this information for the configuration of [external routing]. This option is available only in combination with the <b>TCP</b> protocol.<br/><br/>
Learn more about [how to connect to your .NET service](/dotnet/how-to/access).<br/>
        </td>
	</tr>
    <tr>
  <td><b>verticalAutoscaling</b></td>
		<td><b>Optional.</b> Defines <a href="/dotnet/how-to/create#set-auto-scaling-configuration">custom vertical auto scaling parameters</a>.</td>
	</tr>
        <tr>
  <td><b>minContainers</b></td>
		<td>Defines the minimum number of containers for <a href="/dotnet/how-to/create#horizontal-auto-scaling">horizontal autoscaling</a>.<br/><br/><b>Limitations:</b><br/>Current maximum value = 6.</td>
	</tr>
        <tr>
  <td><b>maxContainers</b></td>
		<td>Defines the maximum number of containers for <a href="/dotnet/how-to/create#horizontal-auto-scaling">horizontal autoscaling</a>.<br/><br/><b>Limitations:</b><br/>Current maximum value = 6.</td>
	</tr>
        <tr>
  <td><b>envSecrets</b></td>
		<td><b>Optional.</b> Defines one or more env variables as a key value map. See env variable <a href="/dotnet/how-to/env-variables#env-variable-restrictions">restrictions</a>.</td>
	</tr>
</tbody>
</table>
