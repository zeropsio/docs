...
To connect to your application with `app` hostname running on port `5000`, simply use `http://app:5000`
...
Once the VPN session is established, you have the secured connection to the project's private network in Zerops. You can access all project services locally by using their hostname. The only difference is that no [environment variables](/dotnet/how-to/env-variables) are available when connected through VPN. To connect to your .NET application in Zerops set the hostname and port e.g. http://app:5000
...
By default, your .NET service is not publicly accessible. To test your application, enable the [public access through zerops.io subdomain](/references/public-access#public-access-through-zeropsio-subdomain).

## Public access through your domain
By default, your .NET service is not publicly accessible. When your application is ready for production or if you want to test it on the production domain, [configure the public access through your domain](/references/public-access#public-access-through-your-domain).
