...
If you triggered the deploy pipeline [manually](/dotnet/how-to/trigger-pipeline#manual-builds-and-deploys-using-zerops-cli) using Zerops CLI, the application artefact is also uploaded to the internal Zerops storage.
...
3. Optionally runs the [init commands](/dotnet/how-to/build-pipeline#init)
...
:::info
If your application needs to be initialized in each runtime container, add [init commands](/dotnet/how-to/build-pipeline#init) to <code>zerops.yml</code>.
:::
...
## Readiness checks
If your application isn't ready to handle requests right after it is started via the [start command](/dotnet/how-to/build-pipeline#start), configure a [readiness check](/dotnet/how-to/build-pipeline#readiness-check) in your `zerops.yml`.
