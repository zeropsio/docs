...
* `go@v1` (Go major version is updated regularly)
...
The base build environment contains Ubuntu 22, the selected version of Go, [Zerops command line tool](/references/cli), `git` and `wget`.
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `buildCommands` are finished, the application build is completed and ready for the [deploy](/go/how-to/build-pipeline#deploy) phase.
...
If you need to upgrade the major Go version, create a new service with the required version and set a new [hostname](/go/how-to/build-pipeline#hostname) in your `zerops.yml`. If the major version is not currently supported, create a new Ubuntu service and download the Go version manually.
...
The base Go environment contains Ubuntu 22, the selected major version of Go, [Zerops command line tool](/references/cli) and `git` and `wget`. To install additional packages or tools add one or more prepare commands:
...
The prepare runtime container contains Ubuntu 22, the selected major version of Go, [Zerops command line tool](/references/cli) and `git` and `wget`.
...
If any `initCommands` fails, it returns an exit code other than 0, but deploy is **not** canceled. After all init commands are finished, regardless of the status code, the application is started. Read the [runtime log] to troubleshoot the error.

#### Single or separated shell instances

You can configure your `initCommands` commands to be run in a single shell instance or multiple shell instances. The format is identical to [build commands].
