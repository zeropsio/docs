...
Zerops provides a customizable build environment for your Go application.
...
Each service configuration contains two sections: **build** and **run**. Both sections are required to build and deploy your Go application in Zerops.
...
The base build environment contains Ubuntu 22, the selected major version of Go, [Zerops command line tool](/references/cli), `npm`, `yarn`, `git` and `npx` tools.
...
To customise your build environment use the [prepare](/go/how-to/build-pipeline#prepare) attribute.
...
2. optional customisation of the base environment defined in the [prepare](/go/how-to/build-pipeline#prepare) attribute
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `build` commands are finished, the application build is completed and ready for the [deploy](/go/how-to/build-pipeline#deploy) phase.
...
If you need to upgrade the major Go version, create a new service with the required version and set a new [hostname](/go/how-to/build-pipeline#hostname) in your `zerops.yml`.

### prepare
_OPTIONAL._ Customises the Go runtime environment by installing additional dependencies or tools to the base build environment.

The base Go environment contains Ubuntu 22, the selected major version of Go, [Zerops command line tool](/references/cli) and `npm`, `yarn`, `git` and `npx` tools. To install additional packages or tools add one or more prepare commands:
...
app:
  ...
  run:
    ...
    # or tolls to the base Go runtime environment.
    prepare:
      - apt-get something
      - curl something else
    ...
...
2. optionaly: [copy selected folders or files from your build container](/go/how-to/build-pipeline#copy-folders-or-files-from-your-build-container)
3. run the `prepare` commands in the defined order
...
The prepare runtime container contains Ubuntu 22, the selected major version of Go, [Zerops command line tool](/references/cli) and `npm`, `yarn`, `git` and `npx` tools.
...
We recommend starting your Go application using `npm start`.
