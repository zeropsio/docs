...
3. Optionally [customises the build environment](/go/how-to/build-process#customise-go-build-environment)
4. Runs the build commands
5. Uploads the application artefact to the internal Zerops storage
6. Optionally [customizes the runtime environment](/go/how-to/customize-runtime)
7. [Deploys your application](/go/how-to/deploy-process)
...
## Customise Go build environment
The default Go build environment contains:
* Ubuntu 22
* selected version of Go defined in `zerops.yml` [build.base](/go/how-to/build-pipeline#base) parameter
* [zCLI](/references/cli/install), Zerops command line tool
* `go`, `git` and `wget`

To install additional packages or tools add one or more [build.prepare](/go/how-to/build-pipeline#prepare) commands to your `zerops.yml`.
...
If any [prepare command](/go/how-to/build-pipeline#prepare) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/go/how-to/logs#build-log) to troubleshoot the error. If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `prepare` commands are finished, your custom build environment is ready for the build phase.
...
1. Content of the [build.base](/go/how-to/build-pipeline#base) and [build.prepare](/go/how-to/build-pipeline#prepare) attributes didn't change from the previous build
...
If any [build command](/go/how-to/build-pipeline#build) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/go/how-to/logs#build-log) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the verbose `-v` option.
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `build` commands are finished, the application build is completed and ready for the [deploy](/go/how-to/deploy-process) phase.
