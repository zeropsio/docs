...
Follow these steps:
1. Add [zerops.yml](/go/how-to/build-pipeline#add-zeropsyml-to-your-repository) to your repository.
...
Follow these steps:
1. Add <a href="/go/how-to/build-pipeline#add-zeropsyml-to-your-repository"><code>zerops.yml</code></a> to your repository.
2. [Install & setup zCLI](/references/cli) the Zerops command line tool.
3. Run `zcli service push` command.

The `zcli service push` command uploads your application code, builds and deploys your application in Zerops.

The command triggers the [build pipeline](/go/how-to/build-pipeline) defined in `zerops.yml`. `zerops.yml` must be in the working directory. The working directory is by default the current directory and can be changed using the `--workingDir` flag.
...
zCLI commands are interactive, when you press enter after `zcli service push`, you will be given a list of your projects to choose from.
...
Follow these steps:
1. Add [zerops.yml](/go/how-to/build-pipeline#add-zeropsyml-to-your-repository) to your repository. Omit the build section.
2. [Install & setup zCLI](/references/cli) the Zerops command line tool.
