...
To ensure that a pipeline is not triggered by your next commit, add `[ci skip]` or `[skip ci]` to the commit message. It is case insensitive.
...
The command triggers the [build pipeline](/go/how-to/trigger-pipeline) defined in `zerops.yml`. `zerops.yml` must be in the working directory. The working directory is by default the current directory and can be changed using the <br/>`--workingDir` flag.
