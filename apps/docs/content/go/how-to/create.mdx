...
### Choose Go version
Following Go major versions are currently supported:
* v1

:::caution
The Go major version is fixed after the service is created. It can't be changed later.
:::
...
:::info
Internal Go service ports can be changed later.
:::
...
#### How to read env variables from Go

Zerops passes all environment variables from all project services when your Go is deployed and started.
...
Read more about how to create and use [environment variables](/go/how-to/env-variables).
...
:::caution
Check if your application is ready for [highly available mode](/go/how-to/scaling#horizontal-scaling). If not, set the maximum containers to **1**. Zerops will never add more containers to your Go service.
:::
...
services:
  -
    # service name
    hostname: app
    # service type and version number in go@1 format
    type: go@1
...
The yaml file describes your future project infrastructure. The project will contain one Go version 1 service with default [auto scaling](/go/how-to/scaling) configuration. Hostname will be set to "app" and the application will listen on the TCP port 8080 that supports the HTTP communication. Following env variables will be configured:
...
Go service with "app" hostname is configured to listen on tcp ports 8080, 8081 and on udp port 9000. Go service will run on version 1 with a custom vertical and horizontal scaling. Following env variables will be configured:
...
At least one service in `services:` section is required. You can create a project with multiple services. The example above contains Go and PostgreSQL services but you can create a `description.yml` with your own combination of [services].
...
<tbody>
	...
	<tr>
  <td><b>type</b></td>
		<td>Specifies the service type <code>go</code> and version.<br/>Set type: <code>go<span>@</span>&lt;version&gt;</code><br/><br/><b>Limitations:</b><br/>Following Go major versions are currently supported: <i>1.22</i></td>
	</tr>
    <tr>
  <td><b>ports</b></td>
...
The <b>port</b> attribute defines the port number. At least one internal port is required for Go service. You can set any port number between <code>10</code> and <code>65435</code>. Ports outside this interval are reserved for internal Zerops systems.
<br/><br/>
...
Learn more about [how to connect to your Go service](/go/how-to/access).<br/>
        </td>
	</tr>
    <tr>
  <td><b>verticalAutoscaling</b></td>
		<td><b>Optional.</b> Defines <a href="/go/how-to/create#set-auto-scaling-configuration">custom vertical auto scaling parameters</a>.</td>
	</tr>
        <tr>
  <td><b>minContainers</b></td>
		<td>Defines the minimum number of containers for <a href="/go/how-to/create#horizontal-auto-scaling">horizontal autoscaling</a>.<br/><br/><b>Limitations:</b><br/>Current maximum value = 6.</td>
	</tr>
        <tr>
  <td><b>maxContainers</b></td>
		<td>Defines the maximum number of containers for <a href="/go/how-to/create#horizontal-auto-scaling">horizontal autoscaling</a>.<br/><br/><b>Limitations:</b><br/>Current maximum value = 6.</td>
	</tr>
        <tr>
  <td><b>envSecrets</b></td>
		<td><b>Optional.</b> Defines one or more env variables as a key value map. See env variable <a href="/go/how-to/env-variables#env-variable-restrictions">restrictions</a>.</td>
	</tr>
</tbody>
</table>
