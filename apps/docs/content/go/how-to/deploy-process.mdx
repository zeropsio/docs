...
If you triggered the deploy pipeline [manually](/go/how-to/trigger-pipeline#manual-builds-and-deploys-using-zerops-cli) using Zerops CLI, the application artefact is also uploaded to the internal Zerops storage.
...
3. Optionally runs the [init commands](/go/how-to/build-pipeline#init)
...
:::info
If your application needs to be initialized in each runtime container, add [init commands](/go/how-to/build-pipeline#init) to <code>zerops.yml</code>.
:::

:::caution
Do not use the <code>init</code> commands for customising your runtime environment. See [how to customise the runtime environment](/go/how-to/deploy-process#customise-go-runtime-environment).
:::
