...
When using Docker services on Zerops, environment variables defined in the `run.envVariables` section are available to the host VM but need to be explicitly passed to Docker containers.

To pass environment variables from Zerops to your Docker containers, use the `-e` flag in your Docker run command:
...
zerops:
  - setup: app
    run:
      ...
      start: docker run -e DB_USER -e DB_PORT --network=host my-application:latest
      ports:
        - port: 8080
          httpSupport: true
```
