...
- **Automated execution**:
  - As part of your `zerops.yml` configuration file
...
Manages scheduled tasks that are defined in your zerops.yml configuration.
...
#### Available sub-commands
- `list`: List all crontabs defined in zerops.yml
- `run`: Execute crontab command defined in zerops.yml
...
#### Required parameters
- `baseName`: The technology and version to install - see the full list of supported [base environments](/zerops-yml/base-list).
...
- As a start command in zerops.yml for services that don't have a natural blocking command
:::
