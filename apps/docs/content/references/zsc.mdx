...
Zsc is a management tool that comes pre-installed in all Zerops containers, providing functionality for container management, scaling, backups, and more. It's available in both runtime and build environments without any additional setup needed.
...
#### action
Execute predefined container actions:

```bash
zsc action [action-name]
```
...
#### backup-current
Create backup of the current stack:

```bash
zsc backup-current
```

Both commands capture:
- Stack configuration
- Service data
