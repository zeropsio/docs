...
project:
  ...
  corePackage: SERIOUS
  # Project location - eu-central (EU, Prague, default) or us-east-1 (US East, New York)
  location: eu-central
...
<table className="w-full my-1.5">
  ...
  <tbody>
    ...
    <tr>
      <td className="w-fit font-semibold">location</td>
      <td className="w-fit">string</td>
      <td className="w-fit">**Optional.** The region where the project will be hosted. Values: **`eu-central`** (EU Central, Prague, default) or **`us-east-1`** (US East, New York). Cannot be changed after project creation. See [Project Location](/features/infrastructure#project-location).</td>
    </tr>
  </tbody>
</table>
