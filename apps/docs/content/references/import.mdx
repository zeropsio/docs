...
services:
  ...
  - hostname: app
    ...
    # Environment variables inside service
    envSecrets:
      # yamlPreprocessor feature
      # Generates a random 32 char string for Laravel APP_KEY
  ...
  - hostname: db
    type: postgresql@16
    # Non High-Availability mode (single instance)
    mode: NON_HA
    # Higher priority means this service is created before others
    priority: 10
...
<Dropdown>
  <DropdownItem title="Additional Structure">
<table className="w-full rounded-lg my-1.5">
  <thead>
    <tr>
      <th className="w-fit">Field</th>
