...
`type: nodejs@latest` is just an example. The functions are available for all runtime types.
:::
...
This function generates a random string of specified length using alphanumeric characters and some special characters (`_`, `-`, `.`). It's useful for creating random passwords, API keys, or any other secret values that need to be unique and unpredictable.
...
This function retrieves a previously stored value from memory. It works in conjunction with setVar and other storage functions to access values that were generated or stored earlier in the configuration process.
...
This function combines the functionality of generateRandomString and setVar. It generates a random string and automatically stores it for later use, making it convenient when you need both operations in one step.
...
<Dropdown>
  <DropdownItem question="Date and Time Format Masks">
