...
Zerops scales KeyDB containers automatically by raising or lowering the hardware resources of the containers. If your database is in [highly available](/keydb/how-to/create#highly-available) mode, each of its 2 containers are scaled individually. The vertical scaling of different hardware resources (CPU, RAM and disk) is also  independent. For example if your database needs more CPU, Zerops will assign more CPU to the container and keep the RAM and disk unchanged.
...
Each KeyDB service has mandatory defined maximal resources. Zerops will never scale above the entered values. If your database is in [highly available](/keydb/how-to/create#highly-available) mode, maximal resources are identical for all containers of the KeyDB service.
...
If you need to limit the cost of the KeyDB service, lower the maximal resources. You can set your own maximal resources when creating a KeyDB service or you can change it later in Zerops GUI. Go to the KeyDB service detail and choose **Automatic scaling configuration** in the left menu.

**// TODO screenshot (KeyDB - Automatic scaling configuration form)**
...
If one of the KeyDB containers needs more resources but there are not enough of them on the underlying machine, a new container with the required hardware resources will be started on another machine. When the new container is ready, it will be added to the KeyDB cluster. The old container will be removed from the cluster and deleted. The data synchronisation to the new container starts immediately after the new container is connected to the cluster.

During this operation, the KeyDB cluster always operates on 2 database containers, the data redundancy is not decreased. You will experience a temporary reduction of performance of the database cluster till the data synchronisation is completed. Then, the KeyDB cluster will return back to the required performance. The time required for the data synchronisation depends on the database size.


#### Single container

If the container of the KeyDB needs more resources but there are not enough of them on the underlying machine, the container will be moved to another machine. During the moving operation the container must be switched off. After the container has moved to another machine, it will be started with the required hardware resources. The time required for the KeyDB container to be moved to another machine depends on the database size.
...
Minimal resources are identical for all containers of the KeyDB service.

When you are experiencing problems with insufficient KeyDB performance or capacity, increase the minimal resources. You can set your own minimal resources when creating a KeyDB service or you can change it later in Zerops GUI. Go to the KeyDB service detail and choose **Automatic scaling configuration** in the left menu.
