...
1. Add <a href="bun/how-to/build-pipeline#add-zeropsyml-to-your-repository">`zerops.yml`</a> to your repository.
...
The command triggers the [build pipeline](bun/how-to/trigger-pipeline) defined in `zerops.yml`. `zerops.yml` must be in the working directory. The working directory is by default the current directory and can be changed using the <br/>`--workingDir` flag.
...
1. Add [zerops.yml](bun/how-to/build-pipeline#add-zeropsyml-to-your-repository) to your repository. Omit the build section.
2. [Install & setup zCLI](/references/cli) the Zerops command line tool.
