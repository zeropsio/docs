...
import dataJson from '@site/static/data.json';
...
<UnorderedCodeList data={dataJson.bun.base} />
...
<p>
  The base build environment contains {dataJson.alpine.default}, the selected
...
See the full list of supported [build base environments](/references/zeropsyml/base-list#runtime-services).
...
- <span>{dataJson.alpine.default}</span>
- <span>{dataJson.ubuntu.default}</span>
...
<UnorderedCodeList data={dataJson.bun.readable} />
...
<p>
  The base runtime environment contains {dataJson.alpine.default}, the
...
<p>
  The base Bun environment contains {dataJson.alpine.default} the selected
...
<p>
  The prepare runtime container contains {dataJson.alpine.default}, the selected major version of Bun, <a href="/references/cli">Zerops command line tool</a> and `npm`,
  `yarn`, `git` and `npx` tools.
</p>
