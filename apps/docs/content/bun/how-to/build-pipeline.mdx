...
<UnorderedCodeList data={data.bunVersions} />
...
See the full list of supported [build base environments](/references/zeropsyml/base-list#Bun).
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `buildCommands` are finished, the application build is completed and ready for the [deploy](bun/how-to/build-pipeline#deploy) phase.
...
<p>
  ...
  selected major version of Bun, Zerops command line tool, `npm`,{' '}
  `yarn`, `git` and `npx` tools.
</p>
...
<p>
  The base Bun environment contains {data.alpineCurrentVersion} the selected
  major version of Bun,{' '}
...
:::caution
The init commands will delay the start of your application each time a new runtime container is started (including the [horizontal scaling](bun/how-to/scaling#horizontal-scaling) or when a runtime container is restarted.
...
We recommend starting your Bun application using `npm start`.
...
| Parameter   | Description                                                                                                                                                                                                                                                                                                                     |
| ----------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **command** | Defines a local command to be run.<br/>The command has access to the same [environment variables](bun/how-to/create#set-environment-variables) as your Bun application.<br/>A single string is required. If you need to run multiple commands create a shell script or, use a multiline format as in the example below. |
