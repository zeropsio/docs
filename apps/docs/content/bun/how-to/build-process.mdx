...
3. Optionally [customises the build environment](/Bun/how-to/build-process#customise-Bun-build-environment)
4. Runs the build commands
5. Uploads the application artefact to the internal Zerops storage
6. Optionally [customizes the runtime environment](/Bun/how-to/customize-runtime)
7. [Deploys your application](/Bun/how-to/deploy-process)
...
<p align="center">
  <img
    src="/img/screenshots/Bun_cancel_running_build.png"
    alt="image"
    width="100%"
    height="auto"
  />
...
## Customise Bun build environment
...
If you prefer the Ubuntu OS instead of Alpine, set the `<a href="/Bun/how-to/build-pipeline#os">build.os</a>` attribute to `ubuntu`. To install additional packages or tools add one or more [build.prepareCommands](/Bun/how-to/build-pipeline#preparecommands) commands to your `zerops.yml`.
...
1. Content of the [build.base](/Bun/how-to/build-pipeline#base) and [build.prepareCommands](/Bun/how-to/build-pipeline#preparecommands) attributes didn't change from the previous build
...
If any [build command](/Bun/how-to/build-pipeline#build) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/Bun/how-to/logs#build-log) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the `--verbose` option.
