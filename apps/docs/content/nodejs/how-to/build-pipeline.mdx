---
title: Configure Node.js build & deploy pipeline
---

Zerops provides a customizable build environment for your Node.js application. 
...
app:
  ...
  build:
    ...
    # or tolls to the base build environment.
    # prepare:
    #   - apt-get something
    #   - curl something else
    ...
    deploy: [ dist, package.json, node_modules ]
...
The top-level element is the **hostname** of your service. A runtime service with the same hostname must exist in Zerops. 
Zerops supports the definition of multiple runtime services in a single `zerops.yml`. This is useful when you use a monorepo. Just add multiple hostnames as the top-level elements:
...
Each service configuration contains two sections: **build** and **run**. Both sections are required to build and deploy your Node.js application in Zerops.
...
The base build environment contains Ubuntu 18, the selected major version of Node.js, [Zerops command line tool](/references/cli), `npm`, `yarn`, `git` and `npx` tools. 

:::info
You can change the base environment when you need to. Just simply modify the `zerops.yml` in your repository.
:::
 
If you need to install more technologies to the build environment, set multiple values as an array. For example:
...
app:
  # ==== how to build your application ====
  build:
    # REQUIRED. Install more technologies to the build environment: 
    base: [nodejs@18,go@1]    
    ...
```
...
To customise your build environment use the [prepare](/nodejs/how-to/build-pipeline#prepare) attribute. 
...
2. optional customisation of the base environment defined in the [prepare](/nodejs/how-to/build-pipeline#prepare) attribute
...
When following syntax is used, each command is triggered in a separate environment context. For example, each shell instance starts in the home directory again. When one command creates an environment variable, it won't be available for the next command.
...
If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `build` commands are finished, the application build is completed and ready for the [deploy](/nodejs/how-to/build-pipeline#deploy) phase.
...
```yml
deploy: [ ./path/to/file.txt, ./path/to/dir/ ]
```

#### How to use wildcard in path
...
By default, Zerops doesn't cache the files in the build container. When the build phase is finished, the files and folders defined in the [deploy](/nodejs/how-to/build-pipeline#deploy) attribute are saved and deployed to your runtime containers. Then, the build container is deleted.
To speed the second and following build up, cache the files that can be used in the next build. We recommend to cache the `node_modules` directory.

The cache attribute supports the [~ wildcard character](/nodejs/how-to/build-pipeline#how-to-use-wildcard-in-path).
...
If you need to upgrade the major Node.js version, create a new service with the required version and set a new [hostname](/nodejs/how-to/build-pipeline#hostname) in your `zerops.yml`.

### prepare
_OPTIONAL._ Customises the Node.js runtime environment by installing additional dependencies or tools to the base build environment.

The base Node.js environment contains Ubuntu 18, the selected major version of Node.js, [Zerops command line tool](/references/cli) and `npm`, `yarn`, `git` and `npx` tools. To install additional packages or tools add one or more prepare commands:
...
app:
  ...
  run:
    ...
    # or tolls to the base Node.js runtime environment.
    prepare:
      - apt-get something
      - curl something else
    ...
...
2. optionaly: [copy selected folders or files from your build container] 
3. run the `prepare` commands in the defined order
...
1. Content of the [build.addToRunPrepare] and `run.prepare` attributes didn't change from the previous deploy
...
The prepare runtime container contains Ubuntu 18, the selected major version of Node.js, [Zerops command line tool](/references/cli) and `npm`, `yarn`, `git` and `npx` tools.
...
Do not use the init commands for customising your runtime environment. Use the [run:prepare] attribute instead. 
:::

#### Command exit code

If any `init` command fails, it returns an exit code other than 0, but deploy is **not** canceled. After all init commands are finished, regardless of the status code, the application is started. Read the [runtime log] to troubleshoot the error.

#### Single or separated shell instances

You can configure your `init` commands to be run in a single shell instance or multiple shell instances. The format is identical to [build commands].
