...
<p>
    <video width="100%" autoplay="true" loop="loop" muted="muted" plays-inline="true">
...
:::caution
Check if your application is ready for [highly available mode](/nodejs/how-to/scaling#horizontal-scaling). If not, set the maximum containers to **1**. Zerops will never add more containers to your Node.js service.
:::
...
services:
  -
    # service name
    hostname: app
    # service type and version number in nodejs@20 format
    type: nodejs@20
...
The yaml file describes your future project infrastructure. The project will contain one Node.js version 20 service with default [auto scaling](/nodejs/how-to/scaling) configuration. Hostname will be set to "app" and the application will listen on the TCP port 3000 that supports the HTTP communication. Following env variables will be configured:
...
Node.js service with "app" hostname is configured to listen on tcp ports 3000, 3001 and on udp port 4000. Node.js service will run on version 20 with a custom vertical and horizontal scaling. Following env variables will be configured:
...
<tbody>
	...
	<tr>
  <td><b>type</b></td>
		<td>Specifies the service type <code>nodejs</code> and version.<br/>Set type: <code>nodejs<span>@</span>&lt;version&gt;</code><br/><br/><b>Limitations:</b><br/>Following Node.js major versions are currently supported: <i>20</i>,<i>18</i>,<i>16</i></td>
	</tr>
    <tr>
  <td><b>ports</b></td>
...
The <b>port</b> attribute defines the port number. At least one internal port is required for Node.js service. You can set any port number between <code>10</code> and <code>65435</code>. Ports outside this interval are reserved for internal Zerops systems.
<br/><br/>
...
Learn more about [how to connect to your Node.js service](/nodejs/how-to/access).<br/>
        </td>
	</tr>
    <tr>
