...
The command triggers the build pipeline defined in [zerops.yml](/nodejs/how-to/build-pipeline). `zerops.yml` must be in the working directory. The working directory is by default the current directory and can be changed using the `--workingDir` flag. 
