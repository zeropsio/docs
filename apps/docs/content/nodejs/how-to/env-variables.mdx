...
Zerops creates several helper variables when a Node.js service is created, e.g. `hostname`, `PATH`. Some helper variables are read-only (`hostname`), others are editable (`PATH`).
