...
Zerops creates several helper variables when a Node.js service is created: `hostname` and `PATH`

You can edit or delete existing env variables by clicking on the menu on the right side of each row. The `hostname` helper variable cannot be changed or deleted.

The changes you've made in the list of env variables needs to be committed. Click on the **Commit x changes** button and Zerops will update the env variables in all containers of your project's services. 
