...
:::info
The application code is available in the `xxx` folder in your build container before the prepare commands are triggered. This allows you to use any file from your application code in your prepare commands (e.g. a configuration file).
:::
...
If any [prepare command](/nodejs/how-to/build-pipeline#prepare) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/references/cli/access-logs) to troubleshoot the error. If the command ends successfully, it returns the exit code 0 and Zerops triggers the following command. When all `prepare` commands are finished, your custom build environment is ready for the build phase.
...
If any [build command](/nodejs/how-to/build-pipeline#build) fails, it returns an exit code other than 0 and the build is canceled. Read the [build log](/references/cli/access-logs) to troubleshoot the error. If the error log doesn't contain any specific error message, try to run your build with the `--verbose` option.
