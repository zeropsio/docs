...
description: Where persistent data lives on Zerops. Runtime containers are replaced on every deploy, so durable files belong in a managed database or in one of the three storage services — Object Storage, Local Storage, or SeaweedFS.
---
...
<table className="w-full my-1.5">
  ...
  <tbody>
    ...
    <tr>
      <td className="w-fit"><b><a href="/seaweedfs/overview">SeaweedFS</a></b></td>
      <td className="w-fit">Network filesystem, mounted or accessed over HTTP by your services</td>
...
<DocCardList
  ...
  items={[
    ...
    {
      type: 'link',
      href: '/seaweedfs/overview',
      label: 'SeaweedFS',
      customProps: {
        icon: Icons['seaweedfs'],
        description: 'Managed distributed filesystem you mount from your services.',
      },
    },
  ]}
/>
