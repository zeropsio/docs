---
title: Configure Your Ubuntu build & deploy pipeline
description: Learn more about how you can configure your Ubuntu application's build & deploy pipeline.
---
...
<UnorderedCodeList data={data.ubuntu.base} />
...
<p>
  The base build environment contains {data.ubuntu.default}, <a href="/references/cli">Zerops command line tool</a>, `git` and `wget`.
</p>
...
If you need to install additional tools in the runtime environment, add them via `prepareCommands` using `zsc add`. For example:
...
<table className="w-full my-1.5">
  ...
  <tbody>
    ...
    <tr>
      ...
      <td className="w-fit"><strong>Optional.</strong> <code>httpSupport</code> defaults to <code>false</code>. Set to <code>true</code> if a web server is running on the port — required for HTTP/S public access via subdomains or custom domains. Available only with the <code>tcp</code> protocol.</td>
    </tr>
  </tbody>
</table>
...
<p>
  The base Ubuntu environment contains {data.ubuntu.default}, <a href="/references/cli">Zerops command line tool</a> and `git` and `wget`. To install additional packages or tools add one or more prepare commands:
</p>
...
<p>
  The prepare runtime container contains {data.ubuntu.default}, <a href="/references/cli">Zerops command line tool</a> and `git` and `wget`.
</p>
  ...
  <tbody>
    <tr>
      ...
      <td className="w-fit">
        ...
        The command has access to the same <a href="/ubuntu/how-to/create#set-secret-environment-variables">environment variables</a> as your Ubuntu application.<br/>
